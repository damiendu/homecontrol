...
function next(){$active.removeClass("active").find("> .dropdown-menu > .active").removeClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded",!1),element.addClass("active").find('[data-toggle="tab"]').attr("aria-expanded",!0),transition?(element[0].offsetWidth,element.addClass("in")):element.removeClass("fade"),element.parent(".dropdown-menu").length&&element.closest("li.dropdown").addClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded",!0),callback&&callback()}var $active=container.find("> .active"),transition=callback&&$.support.transition&&($active.length&&$active.hasClass("fade")||!!container.find("> .fade").length);$active.length&&transition?$active.one("bsTransitionEnd",next).emulateTransitionEnd(Tab.TRANSITION_DURATION):next(),$active.removeClass("in")};var old=$.fn.tab;$.fn.tab=Plugin,$.fn.tab.Constructor=Tab,$.fn.tab.noConflict=function(){return $.fn.tab=old,this};var clickHandler=function(e){e.preventDefault(),Plugin.call($(this),"show")};$(document).on("click.bs.tab.data-api",'[data-toggle="tab"]',clickHandler).on("click.bs.tab.data-api",'[data-toggle="pill"]',clickHandler)}(jQuery),+function($){"use strict";function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.affix"),options="object"==typeof option&&option;data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})}var Affix=function(element,options){this.options=$.extend({},Affix.DEFAULTS,options),this.$target=$(this.options.target).on("scroll.bs.affix.data-api",$.proxy(this.checkPosition,this)).on("click.bs.affix.data-api",$.proxy(this.checkPositionWithEventLoop,this)),this.$element=$(element),this.affixed=null,this.unpin=null,this.pinnedOffset=null,this.checkPosition()};Affix.VERSION="3.3.6",Affix.RESET="affix affix-top affix-bottom",Affix.DEFAULTS={offset:0,target:window},Affix.prototype.getState=function(scrollHeight,height,offsetTop,offsetBottom){var scrollTop=this.$target.scrollTop(),position=this.$element.offset(),targetHeight=this.$target.height();if(null!=offsetTop&&"top"==this.affixed)return offsetTop>scrollTop?"top":!1;if("bottom"==this.affixed)return null!=offsetTop?scrollTop+this.unpin<=position.top?!1:"bottom":scrollHeight-offsetBottom>=scrollTop+targetHeight?!1:"bottom";var initializing=null==this.affixed,colliderTop=initializing?scrollTop:position.top,colliderHeight=initializing?targetHeight:height;return null!=offsetTop&&offsetTop>=scrollTop?"top":null!=offsetBottom&&colliderTop+colliderHeight>=scrollHeight-offsetBottom?"bottom":!1},Affix.prototype.getPinnedOffset=function(){if(this.pinnedOffset)return this.pinnedOffset;this.$element.removeClass(Affix.RESET).addClass("affix");var scrollTop=this.$target.scrollTop(),position=this.$element.offset();return this.pinnedOffset=position.top-scrollTop},Affix.prototype.checkPositionWithEventLoop=function(){setTimeout($.proxy(this.checkPosition,this),1)},Affix.prototype.checkPosition=function(){if(this.$element.is(":visible")){var height=this.$element.height(),offset=this.options.offset,offsetTop=offset.top,offsetBottom=offset.bottom,scrollHeight=Math.max($(document).height(),$(document.body).height());"object"!=typeof offset&&(offsetBottom=offsetTop=offset),"function"==typeof offsetTop&&(offsetTop=offset.top(this.$element)),"function"==typeof offsetBottom&&(offsetBottom=offset.bottom(this.$element));var affix=this.getState(scrollHeight,height,offsetTop,offsetBottom);if(this.affixed!=affix){null!=this.unpin&&this.$element.css("top","");var affixType="affix"+(affix?"-"+affix:""),e=$.Event(affixType+".bs.affix");if(this.$element.trigger(e),e.isDefaultPrevented())return;this.affixed=affix,this.unpin="bottom"==affix?this.getPinnedOffset():null,this.$element.removeClass(Affix.RESET).addClass(affixType).trigger(affixType.replace("affix","affixed")+".bs.affix")}"bottom"==affix&&this.$element.offset({top:scrollHeight-height-offsetBottom})}};var old=$.fn.affix;$.fn.affix=Plugin,$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},null!=data.offsetBottom&&(data.offset.bottom=data.offsetBottom),null!=data.offsetTop&&(data.offset.top=data.offsetTop),Plugin.call($spy,data)})})}(jQuery),function(root,factory){"function"==typeof define&&define.amd?define(["angular","less"],factory):factory(root.angular,root.less)}(this,function(angular,less){var ngLess=angular.module("ngLess",[]);ngLess.factory("styles",[function(){return[]}]),ngLess.directive("head",["styles","$compile","$interpolate",function(styles,$compile,$interpolate){return{restrict:"E",link:function(scope,element){scope.styles=styles;var html='<style ng-repeat="style in styles">{{style.css}}</style>';html=html.replace("{{",$interpolate.startSymbol()),html=html.replace("}}",$interpolate.endSymbol()),element.append($compile(html)(scope))}}}]),ngLess.directive("link",["styles","$http",function(styles,$http){return{restrict:"E",link:function(scope,element){if("stylesheet/less"===element.attr("rel")&&element.attr("href")){var style;element.on("$destroy",function(){return style?(styles.splice(styles.indexOf(style),1),void scope.$apply()):void console.log("no style")}),$http.get(element.attr("href")).then(function(response){if(!element.parent().length)throw new Error("Element was removed.");return less.render(response.data)}).then(function(renderResult){if(!element.parent().length)throw new Error("Element was removed.");style={css:renderResult.css},styles.push(style)})["catch"](function(error){if("Element was removed."!==error.message)throw error})}}}}])}),angular.module("cgNotify",[]).factory("notify",["$timeout","$http","$compile","$templateCache","$rootScope",function($timeout,$http,$compile,$templateCache,$rootScope){var startTop=10,verticalSpacing=15,defaultDuration=1e4,defaultTemplateUrl="angular-notify.html",position="center",container=document.body,maximumOpen=0,messageElements=[],openNotificationsScope=[],notify=function(args){"object"!=typeof args&&(args={message:args}),args.duration=args.duration?args.duration:defaultDuration,args.templateUrl=args.templateUrl?args.templateUrl:defaultTemplateUrl,args.container=args.container?args.container:container,args.classes=args.classes?args.classes:"";var scope=args.scope?args.scope.$new():$rootScope.$new();if(scope.$position=args.position?args.position:position,scope.$message=args.message,scope.$classes=args.classes,scope.$messageTemplate=args.messageTemplate,maximumOpen>0)for(var numToClose=openNotificationsScope.length+1-maximumOpen,i=0;numToClose>i;i++)openNotificationsScope[i].$close();$http.get(args.templateUrl,{cache:$templateCache}).success(function(template){var templateElement=$compile(template)(scope);if(templateElement.bind("webkitTransitionEnd oTransitionEnd otransitionend transitionend msTransitionEnd",function(e){("opacity"===e.propertyName||0===e.currentTarget.style.opacity||e.originalEvent&&"opacity"===e.originalEvent.propertyName)&&(templateElement.remove(),messageElements.splice(messageElements.indexOf(templateElement),1),openNotificationsScope.splice(openNotificationsScope.indexOf(scope),1),layoutMessages())}),args.messageTemplate){for(var messageTemplateElement,i=0;i<templateElement.children().length;i++)if(angular.element(templateElement.children()[i]).hasClass("cg-notify-message-template")){messageTemplateElement=angular.element(templateElement.children()[i]);break}if(!messageTemplateElement)throw new Error("cgNotify could not find the .cg-notify-message-template element in "+args.templateUrl+".");messageTemplateElement.append($compile(args.messageTemplate)(scope))}angular.element(args.container).append(templateElement),messageElements.push(templateElement),"center"===scope.$position&&$timeout(function(){scope.$centerMargin="-"+templateElement[0].offsetWidth/2+"px"}),scope.$close=function(){templateElement.css("opacity",0).attr("data-closing","true"),layoutMessages()};var layoutMessages=function(){for(var j=0,currentY=startTop,i=messageElements.length-1;i>=0;i--){var shadowHeight=10,element=messageElements[i],height=element[0].offsetHeight,top=currentY+height+shadowHeight;element.attr("data-closing")?top+=20:currentY+=height+verticalSpacing,element.css("top",top+"px").css("margin-top","-"+(height+shadowHeight)+"px").css("visibility","visible"),j++}};$timeout(function(){layoutMessages()}),args.duration>0&&$timeout(function(){scope.$close()},args.duration)}).error(function(data){throw new Error("Template specified for cgNotify ("+args.templateUrl+") could not be loaded. "+data)});var retVal={};return retVal.close=function(){scope.$close&&scope.$close()},Object.defineProperty(retVal,"message",{get:function(){return scope.$message},set:function(val){scope.$message=val}}),openNotificationsScope.push(scope),retVal};return notify.config=function(args){startTop=angular.isUndefined(args.startTop)?startTop:args.startTop,verticalSpacing=angular.isUndefined(args.verticalSpacing)?verticalSpacing:args.verticalSpacing,defaultDuration=angular.isUndefined(args.duration)?defaultDuration:args.duration,defaultTemplateUrl=args.templateUrl?args.templateUrl:defaultTemplateUrl,position=angular.isUndefined(args.position)?position:args.position,container=args.container?args.container:container,maximumOpen=args.maximumOpen?args.maximumOpen:maximumOpen},notify.closeAll=function(){for(var i=messageElements.length-1;i>=0;i--){var element=messageElements[i];element.css("opacity",0)}},notify}]),angular.module("cgNotify").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("angular-notify.html","<div class=\"cg-notify-message\" ng-class=\"[$classes, \n    $position === 'center' ? 'cg-notify-message-center' : '',\n    $position === 'left' ? 'cg-notify-message-left' : '',\n    $position === 'right' ? 'cg-notify-message-right' : '']\"\n    ng-style=\"{'margin-left': $centerMargin}\">\n\n    <div ng-show=\"!$messageTemplate\">\n        {{$message}}\n    </div>\n\n"+'    <div ng-show="$messageTemplate" class="cg-notify-message-template">\n        \n    </div>\n\n    <button type="button" class="cg-notify-close" ng-click="$close()">\n        <span aria-hidden="true">&times;</span>\n        <span class="cg-notify-sr-only">Close</span>\n    </button>\n\n</div>')}]),function(){var module=angular.module("toggle-switch",["ng"]);module.provider("toggleSwitchConfig",[function(){this.onLabel="On",this.offLabel="Off",this.knobLabel=" ";var self=this;this.$get=function(){return{onLabel:self.onLabel,offLabel:self.offLabel,knobLabel:self.knobLabel}}}]),module.directive("toggleSwitch",["toggleSwitchConfig",function(toggleSwitchConfig){return{restrict:"EA",replace:!0,require:"ngModel",scope:{disabled:"@",onLabel:"@",offLabel:"@",knobLabel:"@"},template:'<div role="radio" class="toggle-switch" ng-class="{ \'disabled\': disabled }"><div class="toggle-switch-animate" ng-class="{\'switch-off\': !model, \'switch-on\': model}"><span class="switch-left" ng-bind="onLabel"></span><span class="knob" ng-bind="knobLabel"></span><span class="switch-right" ng-bind="offLabel"></span></div></div>',compile:function(element,attrs){return attrs.onLabel||(attrs.onLabel=toggleSwitchConfig.onLabel),attrs.offLabel||(attrs.offLabel=toggleSwitchConfig.offLabel),attrs.knobLabel||(attrs.knobLabel=toggleSwitchConfig.knobLabel),this.link},link:function(scope,element,attrs,ngModelCtrl){var KEY_SPACE=32;element.on("click",function(){scope.$apply(scope.toggle)}),element.on("keydown",function(e){var key=e.which?e.which:e.keyCode;key===KEY_SPACE&&scope.$apply(scope.toggle)}),ngModelCtrl.$formatters.push(function(modelValue){return modelValue}),ngModelCtrl.$parsers.push(function(viewValue){return viewValue}),ngModelCtrl.$viewChangeListeners.push(function(){scope.$eval(attrs.ngChange)}),ngModelCtrl.$render=function(){scope.model=ngModelCtrl.$viewValue},scope.toggle=function(){scope.disabled||(scope.model=!scope.model,ngModelCtrl.$setViewValue(scope.model))}}}}])}();var myApp=angular.module("app",["ui.router","ngResource","cgNotify","toggle-switch","app.config"]);myApp.config(function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("layout",{url:"/",templateUrl:"layout/layout.html",controller:"LayoutCtrl"}).state("layout.genericroom",{url:"genericroom",templateUrl:"generic-room/generic-room.html",controller:"GenericRoomCtrl"})}),angular.module("app").controller("GenericRoomCtrl",["$scope","MusicService","FMService","AlarmService","$interval","$stateParams",function($scope,MusicService,FMService,AlarmService,$interval,$stateParams){$scope.alarms=[],$scope.playlists=[],$scope.selectedPlaylist="",$scope.$watch("activeRoom",function(activeRoom){activeRoom.music&&(MusicService.getPlaylists($scope.activeRoom.music.host,function(data){$scope.playlists=data}),MusicService.getCurrentPlayingFile($scope.activeRoom.music.host,function(data){$scope.playingMusic=data}),$scope.getPlayingMusicInterval||($scope.getPlayingMusicInterval=$interval(function(){MusicService.getCurrentPlayingFile($scope.activeRoom.music.host,function(data){$scope.playingMusic=data})},1e4)),activeRoom.music.alarm&&($scope.alarms=AlarmService.getAlarms("?host="+activeRoom.music.host,function(data){$scope.alarms=data})))}),$scope.addAlarm=function(){var id=0;$scope.alarms.length>0&&(id=$scope.alarms[$scope.alarms.length-1].id+1);var newAlarm={id:id,name:"New alarm",on:!1,hour:8,min:0,frequency:{},host:$scope.activeRoom.music.host,playlist:$scope.globalPlaylists[1].file,lastTime:5,on:!0};$scope.alarms.push(newAlarm)},$scope.setSleepMusic=function(){MusicService.setSleep($scope.activeRoom.music.host,$scope.activeRoom.music.plug,$scope.activeRoom.music.sleepMinutes)},$scope.saveAlarms=function(){AlarmService.saveAlarms("?host="+$scope.activeRoom.music.host,$scope.alarms)},$scope.removeAlarm=function(alarm){for(var id=0,i=0;i<$scope.alarms.length;i++)if($scope.alarms[i].id===alarm.id){id=i;break}$scope.alarms.splice(id,1)},$scope.playPlaylist=function(playlist){MusicService.playPlaylist($scope.activeRoom.music.host,playlist)},$scope.turnOnMusic=function(){FMService.turnOn($scope.activeRoom.music.plug)},$scope.turnOffMusic=function(){MusicService.stop($scope.activeRoom.music.host),FMService.turnOff($scope.activeRoom.music.plug)},$scope.previousSong=function(){MusicService.playPrevious($scope.activeRoom.music.host)},$scope.playSong=function(){MusicService.playPause($scope.activeRoom.music.host)},$scope.pauseSong=function(){MusicService.playPause($scope.activeRoom.music.host)},$scope.stopSong=function(){MusicService.stop($scope.activeRoom.music.host)},$scope.nextSong=function(){MusicService.playNext($scope.activeRoom.music.host)},$scope.volumeUp=function(){MusicService.volumeUp($scope.activeRoom.music.host)},$scope.volumeDown=function(){MusicService.volumeDown($scope.activeRoom.music.host)},$scope.turnPlugOn=function(plug){FMService.turnOn(plug)},$scope.turnPlugOff=function(plug){FMService.turnOff(plug)}}]),angular.module("app").controller("LayoutCtrl",["$scope","$state","HomeDefinitionService","MusicService",function($scope,$state,HomeDefinitionService,MusicService){function loadPlaylists(){MusicService.getGlobalPlaylists(function(data){$scope.globalPlaylists=data})}$scope.homeDefinition={},$scope.activeRoom={},loadPlaylists(),HomeDefinitionService.getHomeDefinition(function(data){$scope.homeDefinition=data,$scope.activeRoom=$scope.homeDefinition.rooms[0],$state.go("layout.genericroom")}),$scope.goToRoom=function(room){$scope.activeRoom=room,$state.go("layout.genericroom")}}]),angular.module("app").factory("MusicService",["$http","notify","serverBaseUrl",function($http,notify,serverBaseUrl){return{playNext:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Player.GoTo",id:1,params:{playerid:0,to:"next"}}}).success(function(data,status,headers,config){notify.closeAll(),notify("next")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},shutdown:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"System.Shutdown",id:1}}).success(function(data,status,headers,config){notify.closeAll(),notify("shuting down")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},playPlaylist:function(room,playlist){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",id:1,method:"Player.Open",params:{item:{directory:playlist},options:{shuffled:!0}}}}).success(function(data,status,headers,config){notify.closeAll(),notify("playing : "+playlist)}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},playPrevious:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Player.GoTo",id:1,params:{playerid:0,to:"previous"}}}).success(function(data,status,headers,config){notify.closeAll(),notify("previous")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},playPause:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Player.PlayPause",id:1,params:{playerid:0}}}).success(function(data,status,headers,config){notify.closeAll(),notify("play/pause")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},stop:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Player.Stop",id:1,params:{playerid:0}}}).success(function(data,status,headers,config){notify.closeAll(),notify("stop")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},getGlobalPlaylists:function(callback){$http.get(serverBaseUrl+"/music/playlists").success(function(data,status,headers,config){var objectReturned=data;try{objectReturned=JSON.parse(data)}catch(e){}callback(objectReturned.result&&objectReturned.result.files?objectReturned.result.files:[])}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},getPlaylists:function(room,callback){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Files.GetDirectory",params:{directory:"/storage/music",media:"files"},id:"1"}}).success(function(data,status,headers,config){var objectReturned=data;try{objectReturned=JSON.parse(data)}catch(e){}callback(objectReturned.result&&objectReturned.result.files?objectReturned.result.files:[])}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},volumeUp:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Application.SetVolume",id:"1",params:{volume:"increment"}}}).success(function(data,status,headers,config){notify.closeAll(),notify("volume up")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},volumeDown:function(room){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Application.SetVolume",id:"1",params:{volume:"decrement"}}}).success(function(data,status,headers,config){notify.closeAll(),notify("volume down")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},setSleep:function(host,plug,minutes){$http.post(serverBaseUrl+"/music/sleep",{host:host,plug:plug,sleepMinutes:minutes}).success(function(data,status,headers,config){notify.closeAll(),notify("sleep set")}).error(function(data,status,headers,config){notify.closeAll(),notify("error")})},getCurrentPlayingFile:function(room,callback){$http.post(serverBaseUrl+"/music",{room:room,jsonToSend:{jsonrpc:"2.0",method:"Player.GetItem",params:{properties:["title","album","artist","duration","thumbnail","file","fanart","streamdetails"],playerid:0},id:"AudioGetItem"}}).success(function(data,status,headers,config){var objectReturned=data;try{objectReturned=JSON.parse(data)}catch(e){}objectReturned.result&&objectReturned.result.item?callback(objectReturned.result.item):callback()}).error(function(data,status,headers,config){notify.closeAll(),notify("error getting current playing music")})}}}]),angular.module("app").factory("FMService",["$http","notify","serverBaseUrl",function($http,notify,serverBaseUrl){return{turnOn:function(plug){$http.put(serverBaseUrl+"/plug/status",{status:"1",plug:plug}).success(function(data,status,headers,config){notify.closeAll(),notify("On")}).error(function(data,status,headers,config){notify.closeAll(),notify("Error")})},turnOff:function(plug){$http.put(serverBaseUrl+"/plug/status",{status:"0",plug:plug}).success(function(data,status,headers,config){notify.closeAll(),notify("Off")}).error(function(data,status,headers,config){notify.closeAll(),notify("Error")})}}}]),angular.module("app").factory("AlarmService",["$http","notify","serverBaseUrl",function($http,notify,serverBaseUrl){return{getAlarms:function(params,callback){return $http.get(serverBaseUrl+"/alarms"+params).success(function(data,status,headers,config){callback&&callback(data)}).error(function(data,status,headers,config){notify("An error occured on server please contact administrator")})},deleteAlarm:function(alarm,callback){$http["delete"](serverBaseUrl+"/alarms/"+alarm.id).success(function(data,status,headers,config){callback&&callback(data),notify("This alarm is deleted but will keep ringing until you save")}).error(function(data,status,headers,config){notify("An error occured on server please contact administrator")})},saveAlarms:function(params,alarms,callback){$http.put(serverBaseUrl+"/alarms"+params,alarms).success(function(data,status,headers,config){callback&&callback(data),notify("Alarms are now set")}).error(function(data,status,headers,config){notify("An error occured on server please contact administrator")})}}}]),angular.module("app").factory("HomeDefinitionService",["$http","notify","serverBaseUrl",function($http,notify,serverBaseUrl){return{getHomeDefinition:function(callback){return $http.get(serverBaseUrl+"/homedef").success(function(data,status,headers,config){callback&&callback(data)}).error(function(data,status,headers,config){notify("An error occured on server please contact administrator")})}}}]),angular.module("app").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("generic-room/generic-room.html",'<div><div ng-if="activeRoom.music && activeRoom.music.host" class=roomSection><div class=section-banner><div class=section-title>Music</div></div><div ng-if=activeRoom.music.plug><div class="btn btn-xs btn-success" ng-click=turnOnMusic()><span class="glyphicon glyphicon-off"></span> Turn on</div><div class="btn btn-xs btn-danger" ng-click=turnOffMusic()><span class="glyphicon glyphicon-off"></span> Turn off</div></div><br><br><div class=form-inline><select class=form-control id=form-small ng-model=selectedPlaylist><option ng-repeat="playlist in globalPlaylists" value={{playlist.file}}>{{playlist.label}}</option><option ng-repeat="playlist in playlists" value={{playlist.file}}>{{playlist.label}}</option></select><div class="btn btn-primary btn-xs" ng-click=playPlaylist(selectedPlaylist)><span class="glyphicon glyphicon-play"></span></div></div><br><div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<div class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-minus" ng-click=volumeDown()></span>&nbsp;&nbsp; <span class="glyphicon glyphicon-volume-up"></span>&nbsp;&nbsp; <span class="glyphicon glyphicon-plus" ng-click=volumeUp()></span></div></div><br><div>Sleep : <input id=form-xs type=number class="input-sm form-control" class=input ng-model="activeRoom.music.sleepMinutes"><div class="btn btn-xs btn-primary" ng-click=setSleepMusic()>Set</div></div><br><div><div class="btn btn-primary btn-xs" ng-click=previousSong()><span class="glyphicon glyphicon-step-backward"></span></div><div class="btn btn-primary btn-xs" ng-click=pauseSong()><span class="glyphicon glyphicon-pause"></span></div><div class="btn btn-primary btn-xs" ng-click=playSong()><span class="glyphicon glyphicon-play"></span></div><div class="btn btn-primary btn-xs" ng-click=stopSong()><span class="glyphicon glyphicon-stop"></span></div><div class="btn btn-primary btn-xs" ng-click=nextSong()><span class="glyphicon glyphicon-step-forward"></span></div></div><p ng-if=playingMusic>{{playingMusic.artist}}<br>{{playingMusic.title}}</p></div><div class=roomSection ng-if=activeRoom.music.alarm><div class=section-banner><div class=section-title>Alarms</div></div><div class="form-inline alarm-box" ng-repeat="alarm in alarms"><div class="btn btn-xs btn-danger btn-close float-right" ng-click=removeAlarm(alarm)><span class="glyphicon glyphicon-remove"></span></div><div class=box-wrapper><input id=form-small class="form-control input-sm" ng-model=alarm.name><div class="btn btn-xs form-control"><toggle-switch ng-model=alarm.on></toggle-switch></div><br><input id=form-xs type=number class="input-sm form-control" class=input ng-model="alarm.hour">:<input id=form-xs type=number class="input-sm form-control" class=input ng-model="alarm.min"><div class="btn-xs btn" ng-click="alarm.frequency.monday=!alarm.frequency.monday" ng-class="{\'btn-default\':!alarm.frequency.monday,\'btn-primary\':alarm.frequency.monday}">mo</div><div class="btn-xs btn" ng-click="alarm.frequency.tuesday=!alarm.frequency.tuesday" ng-class="{\'btn-default\':!alarm.frequency.tuesday,\'btn-primary\':alarm.frequency.tuesday}">tu</div><div class="btn-xs btn" ng-click="alarm.frequency.wednesday=!alarm.frequency.wednesday" ng-class="{\'btn-default\':!alarm.frequency.wednesday,\'btn-primary\':alarm.frequency.wednesday}">we</div><div class="btn-xs btn" ng-click="alarm.frequency.thursday=!alarm.frequency.thursday" ng-class="{\'btn-default\':!alarm.frequency.thursday,\'btn-primary\':alarm.frequency.thursday}">th</div><div class="btn-xs btn" ng-click="alarm.frequency.friday=!alarm.frequency.friday" ng-class="{\'btn-default\':!alarm.frequency.friday,\'btn-primary\':alarm.frequency.friday}">fr</div><div class="btn-xs btn" ng-click="alarm.frequency.saturday=!alarm.frequency.saturday" ng-class="{\'btn-default\':!alarm.frequency.saturday,\'btn-primary\':alarm.frequency.saturday}">sa</div><div class="btn-xs btn" ng-click="alarm.frequency.sunday=!alarm.frequency.sunday" ng-class="{\'btn-default\':!alarm.frequency.sunday,\'btn-primary\':alarm.frequency.sunday}">su</div><br><div>Stop after (min): <input id=form-xs type=number class="input-sm form-control" class=input ng-model="alarm.lastTime"> Playlist :<select class=form-control id=form-small ng-model=alarm.playlist><option ng-repeat="playlist in globalPlaylists" value={{playlist.file}}>{{playlist.label}}</option><option ng-repeat="playlist in playlists" value={{playlist.file}}>{{playlist.label}}</option></select></div></div></div><div class="float-left btn-sm btn btn-default" ng-click=saveAlarms()><span class="glyphicon glyphicon-floppy-disk"></span> Save and apply</div><div class="float-right btn btn-sm btn-primary" ng-click=addAlarm()><span class="glyphicon glyphicon-plus"></span> Add alarm</div></div><div class=roomSection ng-if="activeRoom.plugs && activeRoom.plugs.length>0"><div class=section-banner><div class=section-title>Plug</div></div><table class=table-condensed><tr ng-repeat="plug in activeRoom.plugs"><td><b>{{plug.label}} :</b></td><td><div ng-click=turnPlugOn(plug) class="btn btn-xs btn-success"><span class="glyphicon glyphicon-off"></span> On</div></td><td><div ng-click=turnPlugOff(plug) class="btn btn-xs btn-danger"><span class="glyphicon glyphicon-off"></span> Off</div></td></tr></table></div></div>'),$templateCache.put("layout/layout.html",'<div><div class=page-header><div class=page-title>Home control</div></div><ul class="nav nav-tabs navbar-default"><li ng-repeat="room in homeDefinition.rooms track by $index" role=presentation ng-class="{\'active\':activeRoom===room}"><a ng-click=goToRoom(room)>{{room.label}}</a></li></ul><div class=tab-content ui-view></div></div>')}]);